...
struct ninja ninja_interaction(struct tile board[ROWS][COLS], struct ninja ninja);
struct ninja ninja_interaction_2(struct tile board[ROWS][COLS], struct ninja ninja);
struct ninja paintbrush_colour(struct tile board[ROWS][COLS], struct ninja ninja);
struct ninja gameplay_2(struct tile board[ROWS][COLS], 
                struct ninja ninja);
int check_paintbrush(struct tile board[ROWS][COLS], struct ninja ninja);
struct ninja check_unpainted(struct tile board[ROWS][COLS], struct ninja ninja);
void unlock_exits(struct tile board[ROWS][COLS]);
struct statistics stat_calc(struct tile board[ROWS][COLS], struct ninja ninja);
struct ninja unlock_condition(struct tile board[ROWS][COLS], struct ninja ninja);
...
void feature_spawn(struct tile board[ROWS][COLS]) {
    ...
    while (command != 's' && scanf("%c", &command) == 1) {
        ...
        } else if (command == 'w' || command == 'e' || command == 'l' || command == 'g') {
...
void gameplay(struct tile board[ROWS][COLS], struct ninja ninja) {
    ...
    while (ninja.win == 0 && scanf(" %c", &ninja.command) == 1 && ninja.command != 'q') {
...
struct ninja trampoline_interaction(struct tile board[ROWS][COLS], 
    ...
    // Determine trampoline direction
    ...
    int dest_row = ninja.row + dx;
    int dest_col = ninja.col + dy;
    ...
    enum entity entity = board[dest_row][dest_col].entity;

    if (entity == WALL) {
        // Stop in front of trampoline
        ninja.slamtoggle = 1;
    } else if (entity == GLASS) {
        // Break glass but don't move
...
struct ninja northeast_tramp(struct tile board[ROWS][COLS], struct ninja ninja) {
    if (ninja.command == 's' || ninja.command == 'S') {
        int target_row = ninja.row;
        int target_col = ninja.col + 1;
        if (target_col >= COLS || board[target_row][target_col].entity == WALL) {
            ninja.row--;
            ninja.slamtoggle = 1;
        } else if (board[target_row][target_col].entity == GLASS) {
            board[target_row][target_col].entity = EMPTY;
            ninja.row--;
            ninja.slamtoggle = 1;
        } else {
            ninja.command = (ninja.command == 's') ? 'd' : 'D';
            if (ninja.command == 's') {
                ninja.command = 'd';
            } else {
                ninja.command = 'D';
        ...
        int target_row = ninja.row - 1;
        int target_col = ninja.col;
        if (target_row < 0 || board[target_row][target_col].entity == WALL) {
            ninja.col++;
            ninja.slamtoggle = 1;
        } else if (board[target_row][target_col].entity == GLASS) {
            board[target_row][target_col].entity = EMPTY;
